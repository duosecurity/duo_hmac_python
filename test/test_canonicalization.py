...
class TestGenerateCanonicalStringBasics(unittest.TestCase):
  ...
  def test_get_no_parameter(self):
    actual = duo_canonicalize.generate_canonical_string(DATE_STRING, HTTP_GET, API_HOST, API_PATH, None, None, None)
    self.assertEqual(EXPECTED_GET_NO_PARAMS, actual)

  def test_post_no_parameter(self):
    actual = duo_canonicalize.generate_canonical_string(DATE_STRING, HTTP_POST, API_HOST, API_PATH, None, '{}', None)
    self.assertEqual(EXPECTED_POST_NO_PARAMS, actual)

  def test_method_capitalization(self):
    actual = duo_canonicalize.generate_canonical_string(DATE_STRING, HTTP_GET.lower(), API_HOST, API_PATH, None, None, None)
    self.assertEqual(EXPECTED_GET_NO_PARAMS, actual)

  def test_host_capitalization(self):
    actual = duo_canonicalize.generate_canonical_string(DATE_STRING, HTTP_GET, API_HOST.upper(), API_PATH, None, None, None)
    self.assertEqual(EXPECTED_GET_NO_PARAMS, actual)
...
class TestCanonicalizeParameters(unittest.TestCase):
  ...
  test_cases = [
    ...
    ('Two parameters', {b'realname': [b'First Last'], b'username': [b'root']}, 'realname=First%20Last&username=root'),
    ('Mixed "type" parameters', {b'words': [b'First Last'], b'success': [b'true'], b'digit': [b'5']}, 'digit=5&success=true&words=First%20Last'),
    ('Test similar keys', {b'foo_bar': [b'2'], b'foo': [b'1'],}, 'foo=1&foo_bar=2'),
    (
  ...
  def test_parameter_sorting(self):
    test_keys = [b'one', b'two', b'three', b'four', b'five', b'six', b'seven', b'eight', b'nine']
...
class TestCanonicalizeXDuoHeaders(unittest.TestCase):
  empty_test_cases = [
    ('Empty dict of headers', {}),
    ('None dict of headers', None)
  ]
  ...
  def test_header_sorting(self):
    test_keys = ['x-duo-one', 'x-duo-two', 'x-duo-three', 'x-duo-four', 'x-duo-five', 'x-duo-six', 'x-duo-seven']
    random.shuffle(test_keys)
    test_order_1 = {key: key for key in test_keys}
    random.shuffle(test_keys)
    test_order_2 = {key: key for key in test_keys}
